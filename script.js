...
window.addEventListener('onEventReceived', function (obj) {
  ...
  const data = obj.detail.event.data;
    ...
    // Affiche le nom de l'utilisateur et son message
    console.log(`${data.displayName}: ${data.text}`);
    console.log('Badges:', data.badges);

    // Vérifie si le tableau des badges existe
    if (data.badges && Array.isArray(data.badges)) {
      // Cherche le badge 'broadcaster' dans le tableau
      const isBroadcaster = data.badges.some(badge => badge.type === 'broadcaster');

      if (isBroadcaster) {
        console.log('--- Ce message vient du streamer !');
        // Ici, vous pouvez ajouter du code spécifique pour les messages du streamer
      }
    }  
    // handleMessage(data);
});
